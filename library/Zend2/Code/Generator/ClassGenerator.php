...
class ClassGenerator extends AbstractGenerator {
    ...
    public static function fromReflection(ClassReflection $classReflection) {
        ...
        foreach ($classReflection->getMethods() as $reflectionMethod) {
            if ($reflectionMethod->getDeclaringClass()->getName() == $cg->getNamespaceName() . "\\" . $cg->getName()
                    || $reflectionMethod->getDeclaringClass()->getName() == $cg->getName()) {
    ...
    public function addProperty($name, $defaultValue = null, $flags = PropertyGenerator::FLAG_PUBLIC) {
        if (!is_string($name)) {
            throw new Exception\InvalidArgumentException(sprintf(
                            '%s expects string for name', __METHOD__
            ));
        }
    ...
    public function addPropertyFromGenerator(PropertyGenerator $property) {
        ...
        if (isset($this->properties[$propertyName])) {
            throw new Exception\InvalidArgumentException(sprintf(
                            'A property by name %s already exists in this class.', $propertyName
            ));
        }
    ...
    public function addMethodFromGenerator(MethodGenerator $method) {
        ...
        if (isset($this->methods[$methodName])) {
            throw new Exception\InvalidArgumentException(sprintf(
                            'A method by name %s already exists in this class.', $methodName
            ));
        }
