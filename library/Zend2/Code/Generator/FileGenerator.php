...
class FileGenerator extends AbstractGenerator {
    ...
    public static function fromReflectedFileName($filePath, $includeIfNotAlreadyIncluded = true) {
        ...
        if (!$realpath) {
            throw new Exception\InvalidArgumentException(sprintf(
                            'No file for %s was found.', $filePath
            ));
        }
            ...
            for ($lineNum = 1, $count = count($bodyLines); $lineNum <= $count; $lineNum++) {
                if ($lineNum == $classStartLine) {
                    $bodyReturn[] = str_replace(
                            '?', $class->getName(), '/* Zend_Code_Generator_Php_File-ClassMarker: {?} */'
                    );
        ...
        if (($fileReflection->getDocComment() != '')) {
            ...
            for ($lineNum = 1, $count = count($bodyLines); $lineNum <= $count; $lineNum++) {
                if ($lineNum == $docBlock->getStartLine()) {
                    $bodyReturn[] = str_replace(
                            '?', $class->getName(), '/* Zend_Code_Generator_FileGenerator-DocBlockMarker */'
                    );
    ...
    public function setDocBlock($docBlock) {
        ...
        if (is_array($docBlock)) {
            ...
            throw new Exception\InvalidArgumentException(sprintf(
                            '%s is expecting either a string, array or an instance of %s\DocBlockGenerator', __METHOD__, __NAMESPACE__
            ));
        }
    ...
    public function setClass($class) {
        if (is_array($class)) {
            ...
            throw new Exception\InvalidArgumentException(sprintf(
                            '%s is expecting either a string, array or an instance of %s\ClassGenerator', __METHOD__, __NAMESPACE__
            ));
        }
