...
class Handler {
    ...
    /**
     * Folder pobierania plików
     * 
     * @var string 
     */
    static public $uploadDir = 'application/images/uploaded/';
     ...
     * @var string
     */
    static public $thumbnailDir = 'library/components/fileupload/server/php/thumbnails/';
    public $changed_name;

    public function __construct($options = null) {
        $this->_options = array(
            'script_url' => $this->getFullUrl() . '/',
            'upload_dir' => dirname($_SERVER['SCRIPT_FILENAME']) . '/' . self::$uploadDir,
            'upload_url' => $this->getFullUrl() . '/' . self::$uploadDir,
            'param_name' => 'files',
             ...
             */
            /* 'thumbnail' => array(
              'upload_dir' => dirname($_SERVER['SCRIPT_FILENAME']) . '/' . self::$thumbnailDir,
              'upload_url' => $this->getFullUrl() . '/' . self::$thumbnailDir,
              'max_width' => 80,
              'max_height' => 80
              )
             */
    ...
    protected function get_file_objects() {
        return array_values(array_filter(array_map(
                                        array($this, 'get_file_object'), scandir($this->_options['upload_dir'])
                                )));
    }
    ...
    protected function trim_file_name($name, $type, $index) {
        ...
        if ($this->_options['discard_aborted_uploads']) {
            //usunięcie wcześniejszej wersji pliku
            if (is_file($this->_options['upload_dir'] . $file_name))
                unlink($this->_options['upload_dir'] . $file_name);
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null) {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $append_file = !$this->_options['discard_aborted_uploads'] &&
            ...
            if ($file_size === $file->size) {
                if ($this->_options['orient_image']) {
                    $this->orient_image($file_path);
                }
                $file->url = $this->_options['upload_url'] . rawurlencode($file->name);
                foreach ($this->_options['image_versions'] as $version => $options) {
                    if ($this->create_scaled_image($file->name, $options)) {
                        if ($this->_options['upload_dir'] !== $options['upload_dir']) {
                            $file->{$version . '_url'} = $options['upload_url']
                                    . rawurlencode($file->name);
                        } else {
                            clearstatcache();
